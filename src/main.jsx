...
import { CheckIcon, DocumentTextIcon, EllipsisHorizontalIcon, ExclamationTriangleIcon, InformationCircleIcon, MicrophoneIcon, SpeakerWaveIcon } from '@heroicons/react/24/solid'
import './styles.css'
...
function App() {
  ...
  const [recording, setRecording] = useState(false), [busy, setBusy] = useState(false), [shortcut, setShortcut] = useState('Ctrl+Alt+Espace'), [notice, setNotice] = useState(null), [displayNotice, setDisplayNotice] = useState(null), [leaving, setLeaving] = useState(false), [logs, setLogs] = useState([]), [isInfoOpen, setInfoOpen] = useState(false)
  document.body.classList.toggle('toast-body', isToast)
  useEffect(() => { const refresh = async () => { try { const state = await fetch('http://127.0.0.1:8765/state').then(r => r.json()); setRecording(state.recording); setBusy(state.busy); if (document.activeElement?.id !== 'shortcut') setShortcut(state.shortcut); setNotice(state.notice); setLogs(state.logs) } catch {} }; refresh(); const timer = setInterval(refresh, 120); return () => clearInterval(timer) }, [])
  ...
  const toggleRecording = () => fetch(recording ? 'http://127.0.0.1:8765/stop' : 'http://127.0.0.1:8765/start', { method: 'POST', body: '{}' })
  ...
  if (!isToast) return <main className="app-shell"><section className="control-card"><header><p className="eyebrow">VOICE NOTES</p><h1>Commandes et journal</h1><p>Le bouton reste disponible meme si le raccourci clavier ne repond pas.</p></header><button className={`record-button ${recording ? 'is-recording' : ''}`} onClick={toggleRecording} disabled={busy && !recording}><MicrophoneIcon /> {recording ? 'Arreter l enregistrement' : busy ? 'Finalisation en cours...' : 'Demarrer l enregistrement'}</button><div className="shortcut"><label htmlFor="shortcut">Raccourci global</label><div><input id="shortcut" value={shortcut} onChange={e => setShortcut(e.target.value)} /><button onClick={() => fetch('http://127.0.0.1:8765/hotkey', { method: 'POST', body: JSON.stringify({ value: shortcut }) })}>Appliquer</button></div><small>Exemple : Ctrl+Shift+R</small></div><div className="logs" aria-live="polite">{logs.map((line, i) => <p key={i}>{line}</p>)}</div></section></main>
